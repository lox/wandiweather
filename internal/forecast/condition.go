...
// SmokeLevelFromAirQuality derives a visual smoke band from the latest air quality reading.
// AQI is preferred when present; PM2.5 is used as a fallback.
func SmokeLevelFromAirQuality(aqi int, hasAQI bool, pm25 float64) SmokeLevel {
	if hasAQI {
		switch {
		case aqi > 150:
			return SmokeDense
